...
export function HoursTable({ rows }: { rows: readonly HoursRow[] }) {
  return (
    <table className="w-full border-collapse text-sm">
      ...
      <thead>
        <tr className="text-left text-xs font-semibold tracking-wide text-cream-200/60 uppercase">
          ...
          >
            <th
              ...
              className={`py-2 pr-2 text-left font-medium ${
                row.isToday ? "text-flame-600" : "text-cream-100"
              }`}
            >
              <span className="sm:hidden">{DAY_LABELS_SHORT_SQ[row.weekday]}</span>
              <span className="hidden sm:inline">{row.label}</span>
              {row.isToday ? (
                <span className="ml-2 rounded-pill bg-flame-500/15 px-2 py-0.5 text-xs text-flame-600">
                  sot
                </span>
              ) : null}
            </th>
            <td className="tnum py-2 text-cream-200/80">{row.hoursLabel}</td>
          </tr>
        ))}
      </tbody>
    </table>
...
export function ContactCard() {
  return (
    <div className="rounded-card border border-charcoal-800 bg-charcoal-900 p-5">
      ...
      <address className="mt-3 space-y-3 text-sm not-italic text-cream-200/85">
        <p className="flex items-start gap-2">
          <MapPin aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-flame-600" />
          <a
            href={MAPS_URL}
            target="_blank"
        ...
        <p className="flex items-start gap-2 text-xs text-cream-200/65">
          <MapPin aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-flame-600" />
          <span>{RESTAURANT.landmarks}</span>
        </p>
        <p className="flex items-center gap-2">
          <Phone aria-hidden="true" className="h-4 w-4 shrink-0 text-flame-600" />
          <a
        ...
        <p className="flex items-center gap-2">
          <Clock aria-hidden="true" className="h-4 w-4 shrink-0 text-flame-600" />
          <span>Dorëzim brenda 5 km · 25–40 min</span>
        </p>
        <p className="flex items-center gap-2">
          <Send aria-hidden="true" className="h-4 w-4 shrink-0 text-flame-600" />
          <a
      ...
      <p className="mt-4 rounded-xl bg-charcoal-850 p-3 text-xs text-cream-200/70">
